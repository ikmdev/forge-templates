package dev.ikm.binding.forge.template.pattern.nested.java.test;
...
public class PatternNestedJavaIT {
    ...
    public void givenStarterData_whenPatternJavaTemplateIsExecuted_thenPatternBindingFilesAreGenerated() {
        ...
        try (FileWriter fw = new FileWriter(createFilePathInTarget.apply("/test/Binding.java"))) {
            ...
            simpleForge.config(TEMPLATE_DIRECTORY.toPath())
                    ...
                    .template("pattern-nested-java.ftl", new BufferedWriter(fw))
                    .execute();
        } catch (IOException e) {
            e.printStackTrace();
        }
